<template>
  <div id="weilin_comfyui_tools_prompt_ui_container_main">
    ...
    >
      <keep-alive>
        <TagManager :tagManager="tagManager" :key="tagManager" />
      </keep-alive>
    </DraggableWindow>

    <!-- Lora管理窗口 - 使用keep-alive缓存 -->
    <DraggableWindow
      name="loraManager"
    ...
    >
      <keep-alive>
        <LoraManager :loraManager="loraManager" ref="loraManagerRef" :key="loraManager" />
      </keep-alive>
    </DraggableWindow>

    <!-- 历史记录窗口 - 使用keep-alive缓存 -->
    <DraggableWindow
      name="historyManager"
...
<script setup>
  ...
  // 使用defineAsyncComponent实现组件懒加载，优化首屏加载性能
  const PromptBox = defineAsyncComponent(() => import('./view/prompt_box/prompt_index.vue'))
  const TagManager = defineAsyncComponent(() => import('./view/tag_manager/tag_index.vue'))
  const LoraManager = defineAsyncComponent(() => import('./view/lora_manager/lora_index.vue'))
  const HistoryManager = defineAsyncComponent(
    () => import('./view/history_manager/history_index.vue')
  )
  const FloatingBall = defineAsyncComponent(() => import('@/components/FloatingBall.vue'))
  const AiWindow = defineAsyncComponent(() => import('@/view/ai_window/ai_window.vue'))
  const NodeListWindow = defineAsyncComponent(() => import('@/view/node_list/index.vue'))
  const CloudWindow = defineAsyncComponent(() => import('@/view/cloud/index.vue'))
  const LoraStackWindow = defineAsyncComponent(() => import('@/view/lora_manager/lora_stack.vue'))
  const DanbooruManagerWindow = defineAsyncComponent(
    () => import('@/view/danbooru/danbooru_manager.vue')
  )
  const loraDetail = defineAsyncComponent(() => import('@/view/lora_manager/lora_detail.vue'))
