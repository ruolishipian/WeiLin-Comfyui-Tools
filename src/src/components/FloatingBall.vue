<template>
  <!-- 悬浮球 -->
  <div
    class="weilin_prompt_ui_floating-ball"
    :style="computedBallStyle[i - 1]"
  ...
  >
    ...
    <div v-if="showMenu[i - 1]" class="weilin_prompt_ui_menu-container" @mousedown.stop>
      <div class="weilin_prompt_ui_menu-item" @click="handleMenuItemClick('item1')">
        {{ t('floatingBall.promptBox') }}
      </div>
      ...
      <div class="weilin_prompt_ui_menu-item" @click="handleMenuItemClick('item6')">
        {{ t('floatingBall.openNodeListWindow') }}
      </div>
      <div class="weilin_prompt_ui_menu-item" @click="handleMenuItemClick('item7')">
        {{ t('floatingBall.tranToWeb') }}
      </div>
      <div class="weilin_prompt_ui_menu-item" @click="handleMenuItemClick('item8')">
        {{ t('floatingBall.openSetting') }}
      </div>
      <div class="weilin_prompt_ui_menu-item" @click="handleMenuItemClick('item5')">
        {{ t('floatingBall.restoreWindow') }}
      </div>
    </div>
  </div>
...
<script setup>
  ...
  const savedFloatingBallSize = ref(
    parseInt(localStorage.getItem('weilin_prompt_ui_floatingBallSize'), 10) || 66
  )
  ...
  const handleMessage = (event) => {
    ...
    if (event.data.type === 'weilin_prompt_ui_floating_ball_setting') {
      savedFloatingBallCount.value =
        parseInt(localStorage.getItem('weilin_prompt_ui_floatingBallCount'), 10) || 1
      savedFloatingBallSize.value =
        parseInt(localStorage.getItem('weilin_prompt_ui_floatingBallSize'), 10) || 66
      savedFloatingBallHeight.value =
        parseInt(localStorage.getItem('weilin_prompt_ui_floatingBallHeightSize'), 10) ||
        savedFloatingBallSize.value
