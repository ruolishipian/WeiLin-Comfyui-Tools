<template>
  <DraggableWindow
    v-if="isOpen"
    :title="t('lora.title')"
    :position="windows.loraDetail.position"
    :size="windows.loraDetail.size"
    :z-index="windowManager.getZIndex('loraDetail')"
    @update:position="updatePosition('loraDetail', $event)"
    @update:size="updateSize('loraDetail', $event)"
    @active="windowManager.setActiveWindow('loraDetail')"
    @close="closeWindow('loraDetail')"
  >
    <template #default>
      <div class="lora-detail__content" ref="loraContent">
        ...
        <div class="lora-detail__body">
          ...
          <ul class="lora-detail__tags">
            <li
              v-if="loraInfo.type"
              class="lora-detail__tag"
              :class="`-type-${loraInfo.type.toLowerCase()}`"
              :title="t('lora.type')"
            >
              {{ loraInfo.type }}
            </li>
            <li
              v-if="loraInfo.baseModel"
              class="lora-detail__tag"
              :class="`-basemodel-${loraInfo.baseModel.toLowerCase()}`"
              :title="t('lora.baseModel')"
            >
              {{ loraInfo.baseModel }}
            </li>
          </ul>
          ...
          <table class="lora-detail__table">
            <tbody>
              ...
              <tr>
                ...
                <td colspan="2">
                  {{
                    (_t = (_s = loraInfo.raw) === null || _s === void 0 ? void 0 : _s.metadata) ===
                      null || _t === void 0
                      ? void 0
                      : _t.ss_clip_skip
                  }}
                ...
                <tr :class="{ 'is-editing': isEditing[key] }">
                  ...
                  <td>
                    ...
                    <span v-else class="text">{{
                      loraInfo.user_diy_fileds ? loraInfo.user_diy_fileds[key]?.value : ''
                    }}</span>
                  </td>
                  <td class="actions">
                    <button
                    ...
                    <button
                      style="margin-left: 5px; color: #f87171"
                      @click="removeField(key)"
                      class="edit-btn"
                      :title="t('promptBox.settings.delete')"
                    >
              ...
              <tr v-if="trainedWords.length">
                ...
                <td colspan="2">
                  <ul class="word-list">
                    <li
                      v-for="(word, index) in isCollapsed
                        ? trainedWords.slice(0, 10)
                        : trainedWords"
                      :key="'words-' + index"
                      class="word-item"
                      :class="{
                        'is-selected': isWordSelected(word.word),
                        'is-hidden': isCollapsed && index >= 10
                      }"
          ...
          <ul class="lora-detail__images" v-if="loraInfo.images?.length">
            <li
              v-for="(img, index) in loraInfo.images"
              :key="index"
              class="lora-detail__image-item"
            >
              <figure>
