<template>
  <DraggableWindow
    v-if="isOpen"
    :title="t('randomUtils.title')"
    :position="windows.randomRuleSetting.position"
    :size="windows.randomRuleSetting.size"
    :z-index="windowManager.getZIndex('randomRuleSetting')"
    @update:position="updatePosition('randomRuleSetting', $event)"
    @update:size="updateSize('randomRuleSetting', $event)"
    @active="windowManager.setActiveWindow('randomRuleSetting')"
    @close="closeWindow('randomRuleSetting')"
  >
    <template #default>
                ...
                <div class="rule-content">
                  ...
                  <div class="form-group">
                    ...
                    <select v-model="rule.type" class="form-select">
                      <option value="category">
                        {{ t('randomUtils.category') }}
                      </option>
                      <option value="specific">
                        {{ t('randomUtils.specificTags') }}
                      </option>
                    </select>
                  </div>
                  ...
                  <div v-if="rule.type === 'category'" class="form-group">
                    ...
                    <div class="selected-categories-container">
                      ...
                      <div class="selected-categories-content">
                        <!-- 一级分类（没有子分类的选择） -->
                        <div
                          class="selected-category-section"
                          v-if="primaryCategories(rule.tagGroupList).length > 0"
                        >
                          <div class="section-title">
                            {{ t('importDialog.primaryCategories') }}
                          </div>
                        ...
                        <!-- 没有选择任何分类时显示 -->
                        <div
                          v-if="!rule.tagGroupList || rule.tagGroupList.length === 0"
                          class="no-selected"
                        >
                  ...
                  <div v-if="rule.type === 'specific'" class="form-group">
                    ...
                    <div class="tags-input-container">
                      <div
                        v-for="(tag, tagIndex) in rule.specificTags"
                        :key="tagIndex"
                        class="tag-chip"
                      >
                        {{ tag }}
                      ...
                      <input
                        type="text"
                        v-model="rule.tagInput"
                        @keydown.enter.prevent="addTag(rule)"
                        class="tag-input"
      ...
      <div class="actions-section">
        <button class="save-btn" @click="saveSettings" :disabled="!selectedTemplate">
          {{ t('common.save') }}
        </button>
  ...
  <TagGroupSelect ref="tagGroupSelectItem" @sure-select="sureSelectThis" />
</template>
