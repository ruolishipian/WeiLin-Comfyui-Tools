<template>
  <div :class="`${prefix}lora-manager`">
    <div class="lora-manager-top-bar">
      ...
      <button
        :class="`${prefix}refresh-btn`"
        @click="refreshList"
        :title="t('loraManager.refresh')"
      >
        <svg
          :class="[`${prefix}refresh-icon`, { 'is-rotating': isRefreshing }]"
          viewBox="0 0 24 24"
          width="20"
          height="20"
        >
    ...
    <div :class="`${prefix}category-nav`" v-if="!isSearch">
      <button
        v-for="category in Object.keys(folderList)"
        :key="category"
        :class="[`${prefix}category-btn`, { active: currentCategory === category }]"
    ...
    <div v-if="currentCategory != 'all' && !isSearch" :class="`${prefix}subcategory-nav`">
      <button
        v-for="subCategory in Object.keys(selectFolder)"
        :key="subCategory"
    ...
    <div :class="`${prefix}lora-list-container`" ref="scrollContainer" @scroll="handleScroll">
      ...
      >
        <div
          v-for="lora in paginatedLoraList"
          :key="lora.file_path"
          :class="`${prefix}lora-card`"
          ref="loraCardRef"
          @click="openLoraDetail(lora)"
          @mouseover="(e) => handleMouseHover(lora.name, e)"
          @mouseleave="handleMouseLeave"
          style="display: flex; flex-direction: column; min-height: 200px; cursor: pointer"
        >
          <div
            :class="`${prefix}lora-preview`"
            style="
              flex: 1;
              display: flex;
              align-items: center;
          ...
          <div
            ...
            style="
              ...
              display: -webkit-box;
              -webkit-line-clamp: 2;
              line-clamp: 2; /* 标准属性，消除兼容性警告 */
              -webkit-box-orient: vertical;
              overflow: hidden;
              min-height: 40px;
              max-height: 40px; /* 添加固定高度 */
              line-height: 20px; /* 调整行高：40px高度 / 2行 = 20px行高，适配更精准 */
    ...
    <LoraCard
      ref="loraCardItem"
      v-if="showCard"
      :fileNmae="hoveFileName"
      :paddingLeft="paddingLeftValue"
      :paddingTop="paddingTopValue"
      @card-leave="handleEnterLeave"
      @cardenter="handEnterCard"
    />
  </div>
</template>
...
<script setup>
  ...
  const totalPages = computed(() => {
    ...
    if (rootFolder) {
      ...
      const valuesArray =
        rootFolder.all ||
        Object.keys(rootFolder)
          .filter((key) => key !== 'all')
          .map((key) => rootFolder[key])
      return Math.ceil(valuesArray.length / pageSize.value)
    }
    return 1
  })
  ...
  const handleMouseHover = (fileName, event) => {
    if (!showHoverInfo.value || showHoverInfo.value === 'false') {
      return
    } // 如果不显示悬浮信息，直接返回
  ...
  // 处理滚动事件
  const handleScroll = () => {
    if (!scrollContainer.value || isLoadingMore.value || hasLoadedAll.value) {
      return
    }
      ...
      window.postMessage(
        {
          ...
          lora: {
            ...
            trigger_weight: lora.local_info?.triggerWeight
              ? lora.local_info.triggerWeight
              : lora.local_info?.strWeight
                ? lora.local_info.strWeight
                : 1,
  ...
  const addLoraTag = (loraData) => {
    ...
    const modelName = loraData.model_name
    const modelWeight = loraData.local_info?.strengthMin ? loraData.local_info.strengthMin : 1
    const clipWeight = loraData.local_info?.strWeight ? loraData.local_info.strWeight : 1
    const triggerWeight = loraData.local_info?.triggerWeight
      ? loraData.local_info.triggerWeight
      : clipWeight

    // 生成标签 (新格式4参数)
    // 注意：不再传递 triggerWords，触发词由后端从模型元数据自动提取
    const loraTag = `<wlr:${modelName}:${modelWeight}:${clipWeight}:${triggerWeight}>`

    window.postMessage(
      {
        type: 'weilin_prompt_ui_add_lora_tag',
        tag: loraTag
        // 移除 triggerWords，避免重复添加
      },
      '*'
    )
  }
...
<style scoped>
  ...
  .weilin_prompt_ui_search-input:focus {
    outline: none;
    border-color: var(--weilin-prompt-ui-primary-color);
    box-shadow: 0 0 0 2px rgb(var(--weilin-prompt-ui-primary-color), 0.2);
  }
