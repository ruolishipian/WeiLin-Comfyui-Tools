<template>
  <div
    class="lora_catd_content"
    :style="'left: ' + paddingLeft + 'px;top: ' + paddingTop + 'px;'"
  ...
  >
    ...
    <div class="lora-detail__content" ref="loraContent">
      ...
      <div class="lora-detail__body">
          ...
          <tbody>
            ...
            <template v-for="field in editableFields" :key="field.key">
              <tr :class="{ 'is-editing': isEditing[field.key] }">
                ...
                <td class="actions">
                  <button
                    class="copy-btn"
                    @click="copyToClipboard(loraInfo[field.key])"
                    :title="t('lora.copy')"
                  >
            ...
            <template v-for="(field, key) in userEditFields" :key="key">
              <tr :class="{ 'is-editing': isEditing[key] }">
                ...
                <td class="actions">
                  <button
                    class="copy-btn"
                    @click="copyToClipboard(loraInfo.user_diy_fileds[key]?.value)"
                    :title="t('lora.copy')"
                  >
                    <svg class="svg-icon" viewBox="0 0 24 24" width="16" height="16">
                      <path
            ...
            <tr v-if="trainedWords.length">
              ...
              <td colspan="2">
                <ul class="word-list">
                  <li
                    v-for="(word, index) in isCollapsed ? trainedWords.slice(0, 10) : trainedWords"
                    :key="'words-' + index"
                    class="word-item"
                    :class="{
        ...
        <!-- 图片 -->
        <ul
          class="lora-detail__images"
          v-if="loraInfo.images?.length"
