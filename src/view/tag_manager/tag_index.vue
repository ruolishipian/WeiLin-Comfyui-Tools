  ...
  <div class="tag-manager">
      ...
      <div class="weilin-toolbar-top">
        ...
        <div class="search-container">
          <input
            ...
            class="search-input"
            ref="searchInput"
            @input="isSearching = searchQuery.trim().length > 0"
          />
          <!-- 搜索结果 -->
    ...
    <div class="category-tabs">
      ...
      <div class="tabs-wrapper primary-tabs">
        <div class="tabs-scroll">
          <div
            ...
            :style="{
              backgroundColor:
                selectedCategory?.name === category.name ? 'var(--primary-color)' : category.color,
              color:
                selectedCategory?.name === category.name
                  ? '#ffffff'
                  : getContrastColor(category.color),
              width:
                tabSizeConfig.primaryTab.width === 'fit-content'
                  ? 'fit-content'
                  : tabSizeConfig.primaryTab.width + 'px',
          ...
          >
            ...
            <div class="tab-actions" v-if="hoverTabsActionFrist == 'TabID-' + index">
              <button
                ...
                @click.stop="openMoveGroupDialog(category, 1)"
              ...
              <button
                ...
                @click.stop="shareCategory(category)"
                :title="t('tagManager.share')"
              >
                <svg viewBox="0 0 24 24" class="action-icon">
                  <path
          ...
          <div class="group-edit-mode">
            <label>
              ...
              {{
                editGroupCategroy == 1
                  ? t('tagManager.exitEditMode')
                  : t('tagManager.editGroupMode')
              }}
            </label>
          </div>
        </div>
      ...
      <div class="tabs-wrapper group-tabs" v-if="selectedCategory">
        <div class="tabs-scroll">
          <div
            v-for="(group, index) in subCategories"
            :key="'TabsSw-' + index"
            class="tab-item"
            :class="{ active: selectedGroup?.name === group.name }"
            :style="{
              backgroundColor:
                selectedGroup?.name === group.name ? 'var(--primary-color)' : group.color,
              color: selectedGroup?.name === group.name ? '#ffffff' : getContrastColor(group.color),
              width:
                tabSizeConfig.groupTab.width === 'fit-content'
                  ? 'fit-content'
                  : tabSizeConfig.groupTab.width + 'px',
          ...
          >
            ...
            <div class="tab-actions" v-if="hoverTabsActionSecond == 'TabID-' + index">
              <button
                class="action-btn move"
                @click.stop="openMoveGroupDialog(group, 2)"
                :title="t('tagManager.moveGroup')"
              >
                <svg viewBox="0 0 24 24" class="move-icon">
                  <path
              ...
              <button
                class="action-btn share"
                @click.stop="shareCategorySecond(group)"
                :title="t('tagManager.share')"
              >
                <svg viewBox="0 0 24 24" class="action-icon">
                  <path
    ...
    <div class="tags-content">
      <div class="tags-header">
        <div class="tags-title">
          {{ selectedGroup ? selectedGroup.name : t('tagManager.selectCategory') }}
        </div>
        <div class="tags-actions">
          ...
          <!-- 批量分享按钮 -->
          <button
            v-if="isShareTagAction"
            class="share-selected-btn"
            @click="shareSelectedTags"
          ...
          <div class="tag-content" @click="handleTagClick(tag)">
            <div class="tag-main" :style="{ backgroundColor: tag.color || 'transparent' }">
              ...
              <div class="tag-actions">
                <button
                  class="action-btn move"
                  @click.stop="openMoveDialog(tag)"
                  :title="t('tagManager.moveTag')"
                >
                  <svg viewBox="0 0 24 24" class="move-icon">
                    <path
                      d="M13 6v3h8V6h-8zm0 5v3h8v-3h-8zm-8 5v3h8v-3H5zm0-5v3h8v-3H5zm0-5v3h8V6H5z"
    ...
    <div v-if="showCategoryDialog" class="weilin-tools-dialog-overlay">
      <div class="weilin-tools-dialog-content" @mousedown.stop>
        ...
        <div class="weilin-tools-dialog-body">
          <div class="form-group" v-if="categoryType == 'primary'">
            ...
            <input
              ...
              :placeholder="t('tagManager.categoryNamePlaceholder')"
          ...
          <div class="form-group" v-if="categoryType == 'group'">
            ...
            <input
              ...
              :placeholder="t('tagManager.groupNamePlaceholder')"
              @keyup.enter="saveCategory"
            />
          </div>
            ...
            <div class="color-picker">
              ...
              <div class="color-controls">
                ...
                <div class="alpha-control">
                  <input
                    type="range"
                    v-model.number="colorPickerState.alpha"
                    min="0"
                    max="100"
                    @input="updateColor"
                    class="alpha-slider"
        ...
        <div class="weilin-tools-dialog-footer">
          <button class="cancel-btn" @click="closeCategoryDialog">
            {{ t('common.cancel') }}
          </button>
    ...
    <div v-if="showTagDialog" class="weilin-tools-dialog-overlay">
      <div class="weilin-tools-dialog-content" @mousedown.stop>
        ...
        <div class="weilin-tools-dialog-body">
          <div class="form-group">
            ...
            <input
              ...
              :placeholder="t('tagManager.textPlaceholder')"
            />
          </div>
          <div class="form-group">
        ...
        <div class="weilin-tools-dialog-footer">
          <button class="cancel-btn" @click="closeTagDialog">
            {{ t('common.cancel') }}
          </button>
          <button class="confirm-btn" @click="saveTag">
            {{ t('common.confirm') }}
          </button>
        </div>
      </div>
    ...
    <div v-if="showMoveDialog" class="weilin-tools-dialog-overlay">
      <div class="weilin-tools-dialog-content">
        ...
        <div class="weilin-tools-dialog-footer">
          <button class="cancel-btn" @click="showMoveDialog = false">
            {{ t('common.cancel') }}
          </button>
    ...
    <div v-if="showMoveGroupDialog" class="weilin-tools-dialog-overlay">
      <div class="weilin-tools-dialog-content">
        ...
        <div class="weilin-tools-dialog-body">
          <div class="form-group">
            ...
            <select v-model="moveTargetGroupId" class="form-select">
              <option
                v-for="(item, index) in availableGroup"
                :key="'group_move_id' + index"
                :value="item.id_index"
              >
                {{ item.name }}
              </option>
        ...
        <div class="weilin-tools-dialog-footer">
          <button class="cancel-btn" @click="showMoveGroupDialog = false">
            {{ t('common.cancel') }}
          </button>
          <button class="confirm-btn" @click="confirmMoveGroup">
            {{ t('common.confirm') }}
          </button>
        </div>
      </div>
      ...
      <div class="weilin-tools-dialog-content tab-size-weilin-tools-dialog" @mousedown.stop>
        ...
        <div class="weilin-tools-dialog-body">
          ...
          <div class="config-section">
            ...
            <div class="form-group">
              ...
              <div class="width-control">
                <label class="radio-label">
                  <input
                    type="radio"
                    v-model="tabSizeConfig.primaryTab.width"
                    value="fit-content"
                  />
                ...
                <input
                  v-if="tabSizeConfig.primaryTab.width !== 'fit-content'"
                  type="number"
                  v-model.number="tabSizeConfig.primaryTab.width"
                  min="50"
                  max="300"
                  class="number-input"
                />
            ...
            <div class="form-group">
              <label
                >{{ t('tagManager.tabHeight') }} ({{ tabSizeConfig.primaryTab.height }}px)</label
              >
              <input
                type="range"
                v-model.number="tabSizeConfig.primaryTab.height"
                min="20"
                max="60"
                class="range-input"
              />
            ...
            <div class="form-group">
              <label
                >{{ t('tagManager.fontSize') }} ({{ tabSizeConfig.primaryTab.fontSize }}px)</label
              >
              <input
                type="range"
                v-model.number="tabSizeConfig.primaryTab.fontSize"
                min="8"
                max="20"
                class="range-input"
              />
          ...
          <div class="config-section">
            ...
            <div class="form-group">
              ...
              <div class="width-control">
                ...
                <input
                  v-if="tabSizeConfig.groupTab.width !== 'fit-content'"
                  type="number"
                  v-model.number="tabSizeConfig.groupTab.width"
                  min="50"
                  max="300"
                  class="number-input"
                />
            ...
            <div class="form-group">
              ...
              <input
                type="range"
                v-model.number="tabSizeConfig.groupTab.height"
                min="20"
                max="60"
                class="range-input"
              />
            ...
            <div class="form-group">
              <label
                >{{ t('tagManager.fontSize') }} ({{ tabSizeConfig.groupTab.fontSize }}px)</label
              >
              <input
                type="range"
                v-model.number="tabSizeConfig.groupTab.fontSize"
                min="8"
                max="20"
                class="range-input"
              />
        ...
        <div class="weilin-tools-dialog-footer">
          <button class="reset-btn" @click="resetTabSizeConfig">
            {{ t('tagManager.resetDefault') }}
          </button>
          <button class="cancel-btn" @click="closeTabSizeDialog">
            {{ t('common.cancel') }}
          </button>
          <button class="confirm-btn" @click="saveTabSizeConfig">
            {{ t('common.confirm') }}
          </button>
        </div>
      </div>
...
<style scoped>
  ...
  .loading-indicator::before {
    ...
    width: 16px;
    height: 16px;
    border: 2px solid var(--weilin-prompt-ui-secondary-text);
  ...
  .loading-indicator-center::before {
    content: '';
    width: 32px;
    height: 32px;
    border: 3px solid var(--weilin-prompt-ui-secondary-text);
    border-top-color: transparent;
    border-radius: 50%;
    animation: spin 0.8s linear infinite;
  }
