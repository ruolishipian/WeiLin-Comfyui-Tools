    ...
    <div :class="`${prefix}main-content`" :style="{ width: mainContentWidth }">
      ...
      <button class="tag-manager-btn" @click="toggleLabelManager" :title="t(isLabelManagerVisible ? 'controls.hideSidebar' : 'controls.showSidebar')">
        ...
        <div class="action-item">
          <button
            class="tag-manager-btn"
            @click="openBilibili"
            :title="t('controls.tutorialVideo')"
          >
            <svg
              t="1745234292993"
              class="tag-icon"
      ...
      <div style="position: relative" ref="parentCneterBox">
        ...
        >
          ...
          <div
            ...
            :ref="
              (el) => {
                if (el && index === selectedAutocompleteIndex) selectedItemRef = el
              }
            "
          >
      ...
      <div class="prompt-input-translate-area">
        ...
        <div style="display: flex; align-items: center">
          <input
            ...
            @keyup.enter="finishTranslateEnter()"
      ...
      <div class="tokens-container-wrapper">
        ...
        <div v-else class="tokens-container">
          ...
          <!-- 空状态提示 -->
          <div v-else class="empty-tokens-hint">
            <span>{{
              t('promptBox.noTagsHint') || '暂无标签，请在上方输入框输入或从标签管理器添加'
            }}</span>
          </div>
        </div>
      </div>
      ...
      <div class="tag-manager-section">
        ...
        >
          <div class="weilin-comfyui-selection-actions-content">
            <div class="weilin-comfyui-selection-actions-count">
              选中 {{ selectedTokens.length }} 个标签
            </div>
...
<script setup>
  ...
  // 性能优化模块导入
  import { watchManager } from '@/utils/performance/watch/watch-manager.ts'
  import {
    debounceController,
    createDynamicDebounce
  } from '@/utils/performance/debounce/debounce-controller.ts'
  import { cacheManager } from '@/utils/performance/cache/cache-manager.ts'
  import { webWorkerManager } from '@/utils/performance/worker/web-worker-manager.ts'
  import { batchUpdater } from '@/utils/performance/batch/batch-updater.ts'
  ...
  onUnmounted(() => {
    ...
    // 性能优化：清理所有优化模块
    watchManager.clear() // 清理Watch
    debounceController.clear() // 清理防抖
    // cacheManager.clearAll() // 清理缓存(可选)
    webWorkerManager.clear() // 清理Worker任务
  ...
  const handleMessage = (event) => {
    ...
    if (event.data.type === 'weilin_prompt_ui_insertTag') {
      ...
      if (!loraExistsInStack && tagMatch) {
        ...
        // 立即更新 inputText 以显示 LoRA 标签
        const currentText = inputText.value
        if (currentText === '') {
          inputText.value = loraTag
        } else {
          // 移除所有现有的 <wlr:...> 标签后重新添加
          ...
          // 获取所有 LoRA 标签
          const allLoraTags = selectedLoras.value
            .filter((lora) => !lora.hidden)
            .map(
              (lora) =>
                `<wlr:${lora.name}:${lora.weight}:${lora.text_encoder_weight}:${lora.trigger_weight}>`
            )

          // 组合新文本
          if (allLoraTags.length > 0) {
            inputText.value = cleanText ? `${allLoraTags.join(', ')}, ${cleanText}` : allLoraTags.join(', ')
          } else {
