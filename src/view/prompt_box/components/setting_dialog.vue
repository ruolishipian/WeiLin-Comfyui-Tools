  ...
  <Dialog v-model="dialogVisible" :title="t('promptBox.settings.title')">
    <div class="weilin-comfyui-settings-content">
      <div class="weilin-comfyui-settings-sidebar">
        <ul>
          <li
            :class="{ 'weilin-comfyui-active': selectedSetting === 'translator' }"
        ...
        <div v-if="selectedSetting === 'translator'">
          ...
          <div class="weilin-comfyui-translator-settings">
            ...
            <div class="weilin-comfyui-group-box" v-if="settingTranslater == 'network'">
              <div class="weilin-comfyui-group-title">
                {{ t('promptBox.settings.selectOptionNetworkTranslaterTitle') }}
              </div>
              <div class="weilin-comfyui-note-top">
                {{
                  t('promptBox.settings.selectOptionNetworkTranslaterInfo')
              ...
              <div class="weilin-comfyui-setting-item">
                ...
                <select v-model="savedSourceLanguage" class="weilin-comfyui-common-select">
                  <option value="auto">
                    {{ t('promptBox.settings.auto_detect') }}
                  </option>
                  <option value="chinese_simplified">
                    {{ t('promptBox.settings.chinese_simplified') }}
                  </option>
                  <option value="english">
                    {{ t('promptBox.settings.english') }}
                  </option>
                </select>
              </div>
              <div class="weilin-comfyui-setting-item">
            ...
            <div class="weilin-comfyui-group-box" v-if="settingTranslater == 'translater'">
              <div class="weilin-comfyui-group-title">
                {{ t('promptBox.settings.selectOptionPythonTranslaterTitle') }}
              </div>
              ...
              <div class="weilin-comfyui-translater-setting-box">
                <div class="weilin-comfyui-setting-small-titile">
                  {{ t('promptBox.settings.translaterSetting') }}
                </div>
                <div class="weilin-comfyui-setting-item">
                  <label>{{ t('promptBox.settings.chooseTranslaterSetting') }}</label>
                  <select v-model="selectedTranslatorService">
                    <option
                      v-for="(item, index) in translaterSerives"
                      :index="index"
                      :key="'tran-item_' + index"
                      :value="item"
                    >
                      {{ t('translaterService.' + item) }}
                    </option>
                  </select>
                </div>
                <div class="weilin-comfyui-language-selectors">
                  <div class="weilin-comfyui-setting-item">
                    ...
                    <select v-model="sourceLanguage">
                      ...
                      >
                        {{ t('translaterLanguage.' + item.language) }}
                      </option>
                    </select>
                  </div>
                  <div class="weilin-comfyui-setting-item">
              ...
              <div class="weilin-comfyui-tranlater-text-box">
                <div class="weilin-comfyui-setting-small-titile">
                  {{ t('promptBox.settings.testTranslaterTitle') }}
                </div>
                <div class="weilin-comfyui-setting-item">
                  <label
                    >{{ t('promptBox.settings.inputTestTranslater') }} -
                    {{ t('translaterLanguage.' + retLanguageName(targetLanguage)) }}</label
                  >
                  <input
                    type="text"
                    v-model="testTranslaterInputText"
                    :placeholder="t('promptBox.settings.inputTestTranslaterPlaceholder')"
                  />
                </div>
                <div class="weilin-comfyui-setting-item">
                  <label
                    >{{ t('promptBox.settings.outPutTestTranslater') }} -
                    {{ t('translaterLanguage.' + retLanguageName(sourceLanguage)) }}
                  </label>
                  <input
                    type="text"
                    v-model="testTranslaterOutputText"
                    readonly
        ...
        <div v-if="selectedSetting === 'setting_auto_complete_limit'">
          ...
          <div class="weilin-comfyui-floating-ball-settings">
            <div class="weilin-comfyui-setting-item">
              ...
              <input
                type="number"
                v-model.number="saveAutoCompleteLimit"
                min="1"
                max="99999999"
                style="width: 100px"
            ...
            <div class="weilin-comfyui-setting-item">
              ...
              <input
                type="number"
                v-model.number="saveAutoCompleteWidth"
                min="5"
                style="width: 100px"
            ...
            <div class="weilin-comfyui-setting-item">
              ...
              <input
                type="number"
                v-model.number="saveAutoCompleteHeight"
                min="5"
                style="width: 100px"
        ...
        <div v-if="selectedSetting === 'setting_openai_box'">
          ...
          <div class="weilin-comfyui-openai-settings">
            ...
            <div class="weilin-comfyui-config-list">
              <div
                v-for="(config, index) in openaiSettings"
                :key="index"
                class="weilin-comfyui-config-item"
              >
                <div class="weilin-comfyui-config-header">
                  ...
                  <div class="weilin-comfyui-config-actions">
                    <button @click="toggleEditForm(index)">
                      {{
                        editingIndex === index
                          ? t('promptBox.settings.cancel')
                          : t('promptBox.settings.edit')
                      }}
                    </button>
                    <button @click="deleteOpenaiConfig(index)">
                      {{ t('promptBox.settings.delete') }}
                    </button>
                  </div>
                </div>
                <div class="weilin-comfyui-config-form" v-if="editingIndex === index">
                  ...
                  <div class="weilin-comfyui-form-actions">
                    <button @click="saveOpenaiConfig(index)">
                      {{ t('promptBox.settings.save') }}
                    </button>
                  </div>
                </div>
                <div class="weilin-comfyui-config-details" v-else>
                  <div>
                    {{ t('promptBox.settings.openai_api_key') }}:
                    {{ config.api_key ? '******' : t('promptBox.settings.not_set') }}
                  </div>
            ...
            <div class="weilin-comfyui-config-form" v-if="showOpenaiForm">
              ...
              <div class="weilin-comfyui-form-actions">
                <button @click="cancelOpenaiConfig">
                  {{ t('promptBox.settings.cancel') }}
                </button>
                <button @click="addOpenaiNewConfig">
                  {{ t('promptBox.settings.save') }}
                </button>
              </div>
            </div>
            <button
              class="weilin-comfyui-add-button"
              @click="addOpenaiConfig"
              v-if="!showOpenaiForm"
            >
...
<style scoped>
  ...
  .weilin-comfyui-translater-setting-box
    .weilin-comfyui-language-selectors
    .weilin-comfyui-setting-item {
    flex: 1;
  }
