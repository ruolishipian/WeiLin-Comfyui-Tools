  ...
  <div class="cloud-manager">
    ...
    <div class="weilin-toolbar">
      ...
      <div class="current-path">
        <span
          >{{ t('cloudWindow.currentPath') }}:
          {{
            mainPath == 'tags' ? t('cloudWindow.tagDatabase') : t('cloudWindow.danbooruDatabase')
          }}</span
        >
      </div>
    </div>
    ...
    <div class="file-list">
      <button
        v-if="currentPath !== 'tags' && currentPath !== 'danbooru'"
        class="back-btn"
        @click="goBack"
      >
        <svg viewBox="0 0 24 24" width="16" height="16" class="tag-icon">
      ...
      <div v-for="item in fileList" :key="item.path" class="file-item">
        <div class="file-info">
          ...
          <span class="file-type">{{
            item.type == 'dir' ? '文件夹' : item.type == 'file' ? '文件' : '其它未知文件'
          }}</span>
        </div>
