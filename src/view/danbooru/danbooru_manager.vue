  ...
  <div class="danbooru-manager">
    ...
    <div class="action-bar">
      <div class="search-box">
        <input
          v-model="searchQuery"
          type="text"
          :placeholder="t('danbooruManager.searchPlaceholder')"
          @input="debouncedSearch"
        />
        <button class="search-btn" @click="searchTags">
          {{ t('tagManager.search') }}
        </button>
      </div>
      <button class="add-btn" @click="openAddDialog">
        {{ t('danbooruManager.add') }}
      </button>
      <button class="add-btn" @click="openImportDialog">
        {{ t('tagManager.importDanbooru') }}
      </button>
      <button
      ...
      <table style="table-layout: fixed; overflow: auto">
        <thead>
          <tr>
            ...
            <th class="translate-column">
              {{ t('promptBox.translate') }}
            </th>
            <th class="color-column">
              {{ t('tagManager.backgroundColor') }}
            </th>
            <th class="hot-column">
              {{ t('danbooruManager.hot') }}
            </th>
            <th class="aliases-column">
              {{ t('danbooruManager.aliases') }}
            </th>
            <th class="control-column">
              {{ t('danbooruManager.controle') }}
            </th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="tag in tagList" :key="tag.id_index">
            ...
            <td>{{ tag.aliases }}</td>
            <td>
              <button class="edit-btn" @click="openEditDialog(tag)">
                {{ t('common.edit') }}
              </button>
              <button style="margin-left: 10px" class="delete-btn" @click="confirmDelete(tag)">
                {{ t('common.delete') }}
              </button>
            </td>
          </tr>
      ...
      <div class="weilin-tools-dialog-content">
        ...
        <div class="form-group">
          ...
          <input
            ...
            type="text"
            :placeholder="t('danbooruManager.translatePlaceholder')"
          />
        </div>
        <div class="form-group">
          ...
          <div
            class="color-preview"
            :style="{ backgroundColor: getColorById(formData.color_id) }"
          ></div>
        </div>
        <div class="form-group">
          <label>Hot</label>
    ...
    <div class="weilin-tools-dialog" v-if="showBatchDeleteConfirm">
      <div class="weilin-tools-dialog-content">
        ...
        <p>
          {{ t('danbooruManager.sureBatchDeleteTips1') }} {{ selectedItems.length }}
          {{ t('danbooruManager.sureBatchDeleteTips2') }}
        </p>
        <div class="weilin-tools-dialog-actions">
          <button @click="showBatchDeleteConfirm = false">
            {{ t('common.cancel') }}
          </button>
          <button class="delete-btn" @click="batchDeleteTags">
            {{ t('danbooruManager.sureDelete') }}
          </button>
        </div>
      </div>
    </div>
