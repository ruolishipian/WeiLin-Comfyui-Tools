  ...
  <Dialog v-model="dialogVisible" :title="t('danbooruManager.importBatchTitle')">
    <div class="settings-content">
      <div style="margin-bottom: 16px">
        <button @click="cleanAll" class="upload-btn">
          {{ t('importDialog.cleanAll') }}
        </button>
        <button style="margin-left: 10px" @click="exportSQL">
          {{ t('importDialog.dumpSQL') }}
        </button>
        ...
        <input
          type="file"
          ref="danbooruTxtFileInput"
          @change="handleDanbooruTXTUpload"
          accept=".txt,.csv"
          style="display: none"
        />
      </div>
      ...
      <div class="data-container">
        <div v-if="groupSql">
          ...
          <div v-if="subGroupSql">
            ...
            <div class="tag-group-container">
              ...
              >
                <p v-if="isDanbooruFormat(tagItem)">
                  {{ parseDanbooruSQL(tagItem).tag }} -
                  {{ parseDanbooruSQL(tagItem).translate }} ({{ t('danbooruManager.hot') }}:
                  {{ parseDanbooruSQL(tagItem).hot }}, {{ t('danbooruManager.aliases') }}:
                  {{ parseDanbooruSQL(tagItem).aliases }})
                </p>
                <p v-else>{{ parseSQL(tagItem).text }} - {{ parseSQL(tagItem).desc }}</p>
                <div style="display: flex; align-items: center">
                  <div v-if="editingIndex === index">
                    <input v-if="isDanbooruFormat(tagItem)" v-model="editText" placeholder="Tag" />
                    <input v-else v-model="editText" placeholder="Tag" />
                    <input
                      v-if="isDanbooruFormat(tagItem)"
                      v-model="editDesc"
                    ...
                    <input
                      v-if="isDanbooruFormat(tagItem)"
                      v-model="editHot"
                      :placeholder="t('danbooruManager.hot')"
                      type="number"
                    />
                    <input
                      v-if="isDanbooruFormat(tag)"
                      v-model="editAliases"
                      :placeholder="t('danbooruManager.aliases')"
                      type="number"
                    />
                    <button @click="confirmEdit(index)">
                      {{ t('importDialog.sureYes') }}
                    </button>
                    <button style="margin-left: 10px" @click="cancelEdit">
                      {{ t('importDialog.cancel') }}
                    </button>
                  </div>
                  <button v-else @click="startEdit(index)">
                    {{ t('importDialog.edit') }}
                  </button>
                  <button style="margin-left: 10px" @click="deleteTag(index)">
                    {{ t('importDialog.delete') }}
                  </button>
                </div>
              </div>
            </div>
        ...
        <div v-else-if="tagGroups.length > 0">
          ...
          <div class="tag-group-container">
            <div
              v-for="(tagItem, tagIndex) in tagGroups"
              :key="tagIndex"
              style="display: flex; flex-direction: column; align-items: center"
            >
              <p v-if="isDanbooruFormat(tagItem)">
                {{ parseDanbooruSQL(tagItem).tag }} -
                {{ parseDanbooruSQL(tagItem).translate }} (热度:
                {{ parseDanbooruSQL(tagItem).hot }}, 别名: {{ parseDanbooruSQL(tagItem).aliases }})
              </p>
              <p v-else>{{ parseSQL(tagItem).text }} - {{ parseSQL(tagItem).desc }}</p>
              <div style="display: flex; align-items: center">
                <div v-if="editingIndex === tagIndex">
                  <input v-if="isDanbooruFormat(tagItem)" v-model="editText" placeholder="Tag" />
                  <input v-else v-model="editText" placeholder="Tag" />
                  <input
                    v-if="isDanbooruFormat(tagItem)"
...
<script setup>
        ...
        lines.forEach((line) => {
          ...
          if (parts.length >= 2) {
            ...
            if (parts.length === 2) {
              ...
              tag = parts[0].trim()
              aliases = parseInt(parts[1].trim(), 10) || 0
              hot = parseInt(parts[2].trim(), 10) || 0
  ...
  function parseDanbooruSQL(sql) {
    ...
    const match = sql.match(
      /VALUES\s*\(\s*'([^']+)'\s*,\s*'([^']*)'\s*,\s*'([^']*)'\s*,\s*(\d+)\s*,\s*(\d+)\s*\)/
    )
    ...
    if (match) {
      return {
        ...
        hot: parseInt(match[4], 10),
        aliases: parseInt(match[5], 10)
      }
    }
  ...
  function confirmEdit(index) {
    ...
    if (isDanbooruFormat(tagGroups.value[index])) {
      ...
      const hot = parseInt(editHot.value, 10) || 0
      const aliases = parseInt(editAliases.value, 10) || 0

      tagGroups.value[index] =
        `INSERT INTO "danbooru_tag" ("tag", "color_id", "translate", "hot", "aliases") VALUES ('${escapedText}', '${parsed.color_id}', '${escapedDesc}', ${hot}, ${aliases});`
    } else {
      // 更新原有格式的 SQL
  ...
  function generateDanbooruSQL() {
    ...
    const hotValue = parseInt(hot.value, 10) || 0
    const aliasesValue = parseInt(aliases.value, 10) || 0
