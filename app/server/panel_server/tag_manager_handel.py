        ...
        if handler.command == "GET":
            if parsed_path.path == "/tag/get_tag_groups":
                ...
                if group_id:
                    ...
                    handler.end_headers()
                    handler.wfile.write(
                        json.dumps({"error": "Missing group_id parameter"}).encode()
                    )
                ...
                if subgroup_id:
                    ...
                    handler.end_headers()
                    handler.wfile.write(
                        json.dumps({"error": "Missing subgroup_id parameter"}).encode()
                    )
            else:
            ...
            if parsed_path.path == "/tag/add_tag_group":
                ...
                try:
                    add_tag(
                        data["subgroup_id"], data["text"], data["desc"], data["color"]
                    )
                    handler.send_response(200)
                    self._set_headers(handler)
                    handler.end_headers()
                ...
                try:
                    edit_tag(
                        data["id_index"], data["text"], data["desc"], data["color"]
                    )
                    handler.send_response(200)
                    self._set_headers(handler)
                    handler.end_headers()
